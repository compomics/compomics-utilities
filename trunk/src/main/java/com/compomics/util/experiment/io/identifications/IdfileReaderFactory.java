...
public class IdfileReaderFactory {
    ...
    /**
     * Static initializer block that checks for registered IdfileReaders through the Java service loader
     */
    static {
    ...
    /**
     * This method registers a new IdfileReader Class, and the file extension it can read from. Note that the
     * collection of IdfileReaders is keyed by this extension, and similar to the java.util.HashMap syntax, a
     * Class is therefore returned if the extension provided already had an associated Class.
     *
     * @param aReader   Class of the IdfileReader to register.
     * @param aExtension    String with the extension of the file that this IdfileReader implementation can read.
     * @return  Class with the Class that was already previously registered for this extension, or 'null' if
     *          the extension was not yet registered at all.
     */
    ...
    public IdfileReader getFileReader(File aFile, WaitingHandler waitingHandler) throws SAXException, FileNotFoundException, IOException {
        ...
        //@TODO May want to make this more sophisticated, possibly like the DBLoaders in DBToolkit, that get the actual file to read some lines prior to making up their mind; thus constitutes an actual format check rather than an extension check.
        ...
        while (extensions.hasNext()) {
            ...
            if(name.endsWith(extension)) {
                ...
                try {
                    ...
                    logger.error("Unable to find required constructor with single java.io.File parameter in IdfileReader implementation '" + idfileReaderClass.getCanonicalName() + "', matching query extension '" + extension + "'!", nsme);
                } catch(IllegalAccessException iae) {
                    logger.error("Required public constructor with single java.io.File parameter in IdfileReader implementation '" + idfileReaderClass.getCanonicalName() + "', matching query extension '" + extension + "' has incorrect access modifier!", iae);
                } catch(InvocationTargetException ite) {
                    logger.error("Required constructor with single java.io.File parameter in IdfileReader implementation '" + idfileReaderClass.getCanonicalName() + "', matching query extension '" + extension + "' threw an exception!", ite);
                } catch(InstantiationException ie) {
                    logger.error("Required constructor with single java.io.File parameter in IdfileReader implementation '" + idfileReaderClass.getCanonicalName() + "', matching query extension '" + extension + "' inaccessible; probably abstract class?!", ie);
                }
            }
        }
