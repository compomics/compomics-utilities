...
public class MSNumpress {
     ...
     *
     * @param data the data
     * @param cvAccession the accession number
     *
    ...
    public static byte[] encode(double[] data, String cvAccession) {
        ...
        switch (cvAccession) {
            ...
            case ACC_NUMPRESS_LINEAR: {
                ...
                int encodedBytes = MSNumpress.encodeLinear(
                        data,
                        data.length,
                        buffer,
            ...
            case ACC_NUMPRESS_SLOF: {
                ...
                int encodedBytes = MSNumpress.encodeSlof(
                        data,
                        data.length,
                        buffer,
            ...
            case ACC_NUMPRESS_PIC: {
                ...
                int encodedBytes = MSNumpress.encodePic(
                        data,
                        data.length,
                        buffer
                );
     ...
     *
     * ACC_NUMPRESS_LINEAR = "MS:1002312" ACC_NUMPRESS_PIC = "MS:1002313"
     ...
     * ones. If the initial (most significant) half byte is 0x0 or 0xf, the
     * number of such halfbytes starting from the most significant is stored in
     * a halfbyte. This initial count is then followed by the rest of the ints
     * halfbytes, in little-endian order. A count halfbyte c of 0 &le; c &le; 8
     * is interpreted as an initial c 0x0 halfbytes 9 &le; c &le; 15 is
     ...
     *
     * @return the optimal linear fixed point
     */
     ...
     *
     * This encoding is suitable for typical m/z or retention time binary
     * arrays. For masses above 100 m/z the encoding is accurate to at least 0.1
     * ppm.
     *
     * @param data array of double to be encoded
     * @param dataSize number of doubles from data to encode
     * @param result array were resulting bytes should be stored
     * @param fixedPoint the scaling factor used for getting the fixed point
    ...
    /**
     * Decodes data encoded by encodeLinear. Note that the compression discard
     * any information &lt; 1e-5, so data is only guaranteed to be within +-
     * 5e-6 of the original value.
     *
     * Further, values &gt; ~42000 will also be truncated because of the fixed
     * point representation, so this scheme is strongly discouraged if values
     * above might be above this size.
     *
     * Result vector guaranteedly shorter than twice the data length (in nbr of
     * values) returns the number of doubles read.
     *
     ...
     *
     * @return the number of decoded doubles, or -1 if dataSize &lt; 4 or 4 &lt;
     * dataSize &lt; 8
     */
     ...
     *
     * The handleable range is therefore 0 -&gt; 4294967294. The resulting binary
     * is maximally dataSize * 5 bytes, but much less if the data is close to 0
     * on average.
     *
     ...
     * @param dataSize the data size
     *
     * @return the optimal slof fixed point
     */
     ...
     * point representation of this. This is calculated as
     *
     ...
     *
     * result vector length is twice the data length returns the number of
     * doubles read
     *
     * @param data array of bytes to be decoded (need memorycont. repr.)
     * @param dataSize number of bytes from data to decode
